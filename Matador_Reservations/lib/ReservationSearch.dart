/// Allows the restaurant to search through their reservations
/// 
/// Restaurant can search though their reservations by specific creiteria

import 'package:flutter/material.dart';
import 'package:cloud_firestore/cloud_firestore.dart';
import 'addReservation.dart';

/// Allows the restaurant ro search through their list of reservations
/// 
/// This Class will: 
/// - load the reservations so they can be searched 
/// - Allow the restaurant to filter through their list by
///     * All
///     * Name
///     * Time
///     * Date
///     * Table
///     * reservations they have Today
...
class ReservationSearchPageState extends State<ReservationSearch> {
  ...
  void _performSearch() {
    ...
    if (_searchField == 'All') {
      ...
      results =
          _allReservations.where((res) {
            switch (_searchField) {
              case 'Name':
                return res['name'] != null &&
                    res['name'].toString().toLowerCase().contains(
        ...
        child: Column(
          children: [
            DropdownButton<String>(
              value: _searchField,
              items:
                  _searchOptions.map((String field) {
                  ...
                  onPressed: () async {
                    final result = await Navigator.push(
                      ...
                      MaterialPageRoute(
                        builder:
                            (_) =>
                                Addreservation(restaurant: widget.restaurant),
                      ),
                    );
                    if (result == true) {
                  ...
                  _searchResults.isEmpty
                      ...
                      : ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return ListTile(
                            title: Text(
                              '${r['name'] ?? 'Unknown'} - Table ${r['tableId'] ?? 'N/A'}',
                            ),
                            subtitle: Text(
